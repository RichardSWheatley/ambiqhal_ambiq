...
/*
 * Apollo510 (Cortex-M55) DCACHE / CC312 DMA alignment.
 *
 * Every CC312 context that may sit on a cache line shared with a DMA buffer is
 * declared with this alignment, matching am_hal_cc312_aes_context_t and
 * am_hal_cc312_aes_gcm_context_t in the Ambiq HAL. See chacha_driver.c for the
 * full rationale. Defined locally (with a fallback) so this ARM runtime header
 * does not hard-depend on the HAL headers.
 */
...
} __attribute__((aligned(AM_HAL_CC312_DMA_ALIGNMENT))) ChachaContext_t;
