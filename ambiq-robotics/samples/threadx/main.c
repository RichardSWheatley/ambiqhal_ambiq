/*
 * ARB sample (Eclipse ThreadX on Apollo510): differential-drive base.
 *
 * The closed-loop diff-drive node on the ThreadX broker port. Threads:
 *   - control (50 Hz): encoder feedback -> per-wheel PID -> motors, publishes
 *                      /odom and /imu,
 *   - dispatcher      : drains the broker queue, delivering to subscribers,
 *   - command         : stand-in /cmd_vel source (replace with real input).
 *
 * Peripheral bindings reuse the AmbiqSuite direct-HAL bindings. Build with
 * port/threadx/arb.mk inside an AmbiqSuite + ThreadX example.
 *
...
static TX_THREAD g_ctrl_thread, g_disp_thread, g_cmd_thread;
static uint8_t   g_ctrl_stack[STACK_SZ], g_disp_stack[STACK_SZ],
		 g_cmd_stack[STACK_SZ];
...
{
	ULONG t = (ULONG)ms * TX_TIMER_TICKS_PER_SECOND / 1000u;
	return t ? t : 1;
}
...
static void control_entry(ULONG arg)
{
	...
	for (;;) {
		...
		tx_thread_sleep(ms_to_ticks(CONTROL_PERIOD_MS));
	}
}

static void dispatch_entry(ULONG arg)
{
	(void)arg;
	for (;;) {
...
static void command_entry(ULONG arg)
{
	(void)arg;
	for (;;) {
		arb_twist_t tw = {0};
...
/* Called by ThreadX from tx_kernel_enter(). */
void tx_application_define(void *first_unused_memory)
{
	...
	tx_thread_create(&g_disp_thread, "arb_disp", dispatch_entry, 0,
			 g_disp_stack, STACK_SZ, 6, 6, TX_NO_TIME_SLICE,
			 TX_AUTO_START);
	tx_thread_create(&g_ctrl_thread, "arb_ctrl", control_entry, 0,
			 g_ctrl_stack, STACK_SZ, 5, 5, TX_NO_TIME_SLICE,
			 TX_AUTO_START);
	tx_thread_create(&g_cmd_thread, "arb_cmd", command_entry, 0,
			 g_cmd_stack, STACK_SZ, 8, 8, TX_NO_TIME_SLICE,
			 TX_AUTO_START);
}
