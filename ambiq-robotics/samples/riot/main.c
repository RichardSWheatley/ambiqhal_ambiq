/*
 * ARB sample (RIOT OS on Apollo510): differential-drive base.
 *
 * The closed-loop diff-drive node on the RIOT broker port. A control thread and
 * a command thread run alongside the main thread, which drains the broker:
 *   - control (50 Hz): encoder feedback -> per-wheel PID -> motors, publishes
 *                      /odom and /imu,
 *   - command         : stand-in /cmd_vel source,
 *   - main            : dispatch loop delivering to subscribers.
 *
 * Peripheral bindings reuse the AmbiqSuite direct-HAL bindings; build for an
 * Apollo510 BOARD.
 *
...
static char g_ctrl_stack[THREAD_STACKSIZE_MAIN];
static char g_cmd_stack[THREAD_STACKSIZE_MAIN];
...
static void *control_thread(void *arg)
{
	...
	for (;;) {
		...
		xtimer_msleep(CONTROL_PERIOD_MS);
	}
	return NULL;
}
...
int main(void)
{
	...
	thread_create(g_ctrl_stack, sizeof(g_ctrl_stack),
		      THREAD_PRIORITY_MAIN - 1, 0, control_thread, NULL,
		      "arb_ctrl");
	thread_create(g_cmd_stack, sizeof(g_cmd_stack),
		      THREAD_PRIORITY_MAIN + 1, 0, command_thread, NULL,
		      "arb_cmd");

	/* main thread drains the broker */
	for (;;) {
