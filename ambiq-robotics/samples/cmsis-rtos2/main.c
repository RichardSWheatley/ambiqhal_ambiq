/*
 * ARB sample (CMSIS-RTOS2 on Apollo510): differential-drive base.
 *
 * The closed-loop diff-drive node on the CMSIS-RTOS2 broker port. Threads:
 ...
 * port/cmsis-rtos2/arb.mk against any CMSIS-RTOS2 RTOS (e.g. Keil RTX5).
 *
...
int main(void)
{
	...
	osKernelInitialize();
	osThreadNew(dispatch_thread, NULL, NULL);
	osThreadNew(control_thread, NULL, NULL);
	osThreadNew(command_thread, NULL, NULL);
	osKernelStart();

	for (;;) {
		/* kernel started; should not return */
	}
}
