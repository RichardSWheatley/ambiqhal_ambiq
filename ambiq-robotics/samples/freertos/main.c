/*
 * ARB sample (FreeRTOS on Apollo510): differential-drive base, closed loop.
 *
 * Same robot as the bare-metal sample, but the broker runs on the FreeRTOS port
 * and the work is split across tasks to show cross-task pub/sub:
 *
 *   - control task (50 Hz): encoder feedback -> per-wheel PID -> motors,
 *                           publishes /odom and /imu,
 *   - dispatcher task     : drains the broker queue, delivering to subscribers,
 *   - command task        : stand-in command source publishing /cmd_vel
...
static void control_task(void *arg)
{
	...
	for (;;) {
		...
		vTaskDelayUntil(&next, pdMS_TO_TICKS(CONTROL_PERIOD_MS));
	}
}
...
{
	(void)arg;
	/* Demo command source: alternate straight and turning. Replace with a
	 * real input (UART teleop, JAUS bridge, micro-ROS). */
	for (;;) {
		arb_twist_t tw = {0};
...
int main(void)
{
	...
	xTaskCreate(dispatch_task, "arb_disp", 512, NULL, 3, NULL);
	xTaskCreate(control_task,  "arb_ctrl", 1024, NULL, 4, NULL);
	xTaskCreate(command_task,  "arb_cmd",  512, NULL, 2, NULL);
